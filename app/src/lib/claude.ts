...
- "학교알리미", "지도 앱에서 확인" 등 외부 사이트로 떠넘기는 표현 절대 금지 — 우리가 데이터로 풀어줄 것
- nearestStation 정보가 비어있으면 "역세권" 단어 사용 금지 (정보 없음 + 역세권 단언은 모순)
- 비서울 단지("인천", "경기", "부산" 등)에 "서울 평균"·"서울 중상위권" 등 서울 기준 비교 X — 권역 평균 톤으로
- 한국어 조사는 단어 받침에 맞춰 자연스럽게 ('아파트는' OK, '아파트을(를)' 같은 raw 표기 금지)
- 거래 데이터 기간이 N개월이면 상승률 라벨도 "최근 N개월"로 동기화 (9개월 데이터에 "최근 1년" X)
...
## 🏫 학군 (가구별 H2 헤더 차별화 — 1인/딩크는 '학군·교육 환경', 신혼은 '학군 미리 본다면', 학부모는 '아이 키우기엔'/'학군 핵심 체크')
학군/학원 관점. 프롬프트에 주어진 "주변 육아 인프라" 숫자가 있으면 **반드시 그 숫자를 그대로 인용**.
"지도 앱에서 확인", "직접 알아보세요", "학교알리미에서 확인" 같이 외부 사이트로 떠넘기는 표현 **절대 금지** — 우리가 데이터로 풀어드리는 게 본질.
주어진 반경 1.5km 학교 분포·어린이집 카운트만으로 풀어줄 것. 배정 학교는 언급 자체 X.
...
export async function generateFreeDeepSingleReport(
  ...
  const pyeongPrivate = Math.round((area / 3.3058) * 10) / 10;  // 표준 전용 평수
  const pyeongSupply = typicalPublicPyeong(area);                // 공급 평형 (시장 호칭)
  const pricePerPyeong = apartment.latestPrice10k
    ? calcPricePerPyeong(apartment.latestPrice10k, rawArea)      // 평당가는 측정값 기준 (정확)
    : null;
...
- 주변 학교 분포 (카카오 검색, 반경 1.5km, 거리순):
  ...
  → 학군 섹션에서 이 분포를 인용. 배정 학교 안내·학교알리미 떠넘김 금지 (우리 데이터만 사용).
`
    : ''
}
...
export async function generateCompareReport(
  ...
  const apartmentBlocks = apartments
    .map((apt, i) => {
      ...
      const rawArea = apt.latestAreaM2 ?? 84.99;
      const area = standardPrivateArea(rawArea);
      ...
      const pricePerPyeong = apt.latestPrice10k
        ? calcPricePerPyeong(apt.latestPrice10k, rawArea)
        : null;
...
- 기준 면적: 전용 ${area}㎡ (전용 약 ${pyeongPrivate}평 / 공급 ${pyeongSupply}평형)
- 최근 실거래가: ${priceText} (전용 ${area}㎡ 기준)
